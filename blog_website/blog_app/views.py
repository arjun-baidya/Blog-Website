...
def user_register(request):
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            messages.warning(request, 'User Name Taken')
            return redirect('register')
...
def post_blog(request):
    if request.method == 'POST':
        ...
        blog = Blog(title=title, desc=description, user_id=request.user)
        blog.save()
...
def edit(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/')
    return render(request, 'edit_blog.html', {'editblog': editblog})
